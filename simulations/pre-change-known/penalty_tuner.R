...
run_simulation <- function(p, REPS, seed = 42, diff_thres = F) {
  ...
  max1e6 <- sapply(res, function (r) max(r$maxs))
  output <- rbind(output,
                  data.frame(sim = 1:REPS, algo = "FOCuS 10",  est = cp, max1e3=max1e3,max1e4=max1e4,max1e5=max1e5,max1e6=max1e6, real = p$changepoint, N = p$N, threshold = p$threshold))
...
save(thresholds, file = "simulations/pre-change-known/thresholds.RData")
