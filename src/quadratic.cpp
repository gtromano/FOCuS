...
// careful, this function takes by reference and actually modifies the intervals
// by finding on which domain the quadratics are smallest
void get_min_of_two_quadratics_V1 (Quadratic& q1, Quadratic& q2) {
...
// and try to return the updated cost with a new line added in it
// to be used with std::move() after testing
void get_min_of_cost_V1(std::list<Quadratic>& cost, Quadratic& newq) {
  for (auto& q:cost)
    get_min_of_two_quadratics_V1(q, newq);
...
// to use with std::move() in order to speed up times
void invert_quadratic_v1(Quadratic& q) {
  q.a = - q.a;
  q.b = - q.b;
  q.c = - q.c;
}
...
// add std::move()
// this is simply get min_of_cost but with the inverted coefficients
void get_max_of_cost_V1(std::list<Quadratic>& cost, Quadratic& newq) {

  for (auto& q:cost)
    invert_quadratic_v1(q);
  
  get_min_of_cost_V1(cost, newq); // add std::move
